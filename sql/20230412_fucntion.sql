...
-- 00010
select '10', lpad('10', 10, '0'), rpad('11', 5, '0') from dual; 
-- lpad : 맨뒤를 10으로 설정, 10자리수, 맨뒤 10을 제외하고 그 앞으로 0을 채운다
-- rpad : 맨앞를 11으로 설정, 5자리수, 맨앞 11을 제외하고 그 뒤로 0을 채운다
...
select next_day(sysdate, 1) from dual; -- 다음주의 첫 날짜
select last_day(sysdate) from dual; -- 지금으로부터의 마지막 날짜
select round(sysdate) from dual; --24시를 기준으로 했을때 오후 12시가 넘어가면 반올림 됨
...
insert into emp values(9999,'SON', 'MANAGER', 7782, to_date('04-2023-12', 'MM_YYYY_DD'),4000, null,10);
select * from emp;
...
-- decode 함수 : 동등비교임. 동등 비교 연산 후 결과를 출력하는 함수 switch~case와 비슷
select ename, deptno,
        decode(deptno,
                10, 'ACCOUNTING',
                20, 'RESEARCH',
...
from emp;
-- 위랑 아래 같은거지만 president가 null값 나오기때문에 밑에걸로 사용했음
select ename, job, sal,
nvl(decode (job,
             'ANALYST', sal*1.05, 
            'SALESMAN', sal*1.1,
...
-- 커미션의 총합
select sum(comm), count(comm), avg(comm) -- null값이 있다면 그 값은 배제된다.
from emp;
...
select count(distinct job) -- 중복되는 데이터 제외하고 개수를 카운트
from emp;
...
select deptno, count(*) as "사원의 수",
                ...
                max(sal) as "최고 급여액",
                min(sal) as "최소 급여액",
                count(comm) as "커미션을 받는 사원수" -- count는 개수를 반영.
                
from emp
group by deptno
-- 그룹의 평균 급여가 2000이상인
--having avg(sal)>=2000 -- 그룹핑된 데이터의 집합함수의 결과를 조건식으로 만들때

--총 급여액이 10000을 넘어가는 그룹을 찾고싶다.
